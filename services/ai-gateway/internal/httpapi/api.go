...
func New(verifier *identity.Verifier, router *gateway.Router, logger *slog.Logger) http.Handler {
	...
	api := &API{router: router, logger: logger}
	...
	root.Handle("/", verifier.Authenticate([]string{"ai.use"}, protected))
	return root
}

func (api *API) models(writer http.ResponseWriter, request *http.Request) {
	claims, ok := validDelegation(request)
	if !ok {
		writeError(writer, http.StatusForbidden, "invalid_delegation", "AI delegation policy is missing.")
		return
	}
	writeJSON(writer, http.StatusOK, map[string]any{"object": "list", "data": api.router.Models(claims.Models)})
}
...
func (api *API) chatCompletions(writer http.ResponseWriter, request *http.Request) {
	...
	started := time.Now()
	result, err := api.router.Complete(request.Context(), body, claims.Models, httpx.RequestIDFromContext(request.Context()))
	if err != nil {
...
func validDelegation(request *http.Request) (identity.Claims, bool) {
	claims, ok := identity.FromContext(request.Context())
	return claims, ok && claims.Audience == "ai-gateway" && claims.AuthorizedParty != "" && len(claims.Models) != 0
}
