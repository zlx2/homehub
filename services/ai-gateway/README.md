...
- It accepts only a short-lived IAM Ed25519 access token with audience
  `homehub-ai-gateway` and a non-empty authorized party (`azp`).
- `ai.model.fast`, `ai.model.reasoning`, and `ai.model.coding` are the signed
  per-model allowlist. `system.root` can use every configured alias.
